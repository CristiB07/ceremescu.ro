<?php
// NOTE: settings.php, common.php și paginator.class.php sunt deja incluse de page.php
...
// Folosește prepared statements pentru SQL injection prevention
$stmt = mysqli_prepare($conn, "SELECT * FROM magazin_produse WHERE produs_limba=? AND produs_categorie=?");
mysqli_stmt_bind_param($stmt, 'ss', $lang, $category);
...
$stmt2 = mysqli_prepare($conn, "SELECT * FROM magazin_produse WHERE produs_limba=? AND produs_categorie=? ORDER BY produs_nume ASC $pages->limit");
mysqli_stmt_bind_param($stmt2, 'ss', $lang, $category);
...
?>
    <nav aria-label="<?php echo htmlspecialchars($strYouAreHere, ENT_QUOTES, 'UTF-8')?>:" role="navigation">
        <ul class="breadcrumbs">
            ...
            <li><a href="<?php echo htmlspecialchars($strSiteURL."/shop/".$url, ENT_QUOTES, 'UTF-8')?>"><?php echo htmlspecialchars($category, ENT_QUOTES, 'UTF-8')?></a></li>
