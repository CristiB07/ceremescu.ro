...
<body onLoad=\"setTimeout('delayer()', 1500)\">";
    ...
    // Sanitize field name - only alphanumeric and underscore
...
if (isset($_POST['action']) && $_POST['action'] == 'image') 
{
   // ---------- IMAGE UPLOAD ----------
    ...
    if ($handle->uploaded) {
        ...
        if ($handle->processed) {
            $safe_filename = htmlspecialchars($handle->file_dst_name, ENT_QUOTES, 'UTF-8');
            $safe_url = htmlspecialchars($url_image_directory, ENT_QUOTES, 'UTF-8');
            $safe_field = htmlspecialchars($field, ENT_QUOTES, 'UTF-8');
			echo "<script>setTimeout(\"document.getElementById('src').value='" . $safe_url . "/" . $safe_filename . "'\", 200)</script>";
			echo "<script>setTimeout(\"document.getElementById('img').src='" . $safe_url . "/" . $safe_filename . "'\", 200)</script>";
			echo "<script>setTimeout(\"parent.document.getElementById('" . $safe_field . "').value='" . $safe_filename . "'\", 200)</script>";
            // everything was fine !
           // echo '<p class="result">';
...
<body>
    ...
    <div class="grid-x grid-padding-x">
        <div class="large-6 medium-6 cell">
            ...
            <form action="" enctype="multipart/form-data" method="post">
                ...
                <input type="submit" id="insert" name="insert" class="button round"
                    value="<?php echo $strUploadImage?>" />
            </form>
        </div>
        <div class="large-6 medium-6 cell">
   ...
   $allow = array('jpg','jpeg', 'JPEG', 'JPG', 'png', 'PNG', 'gif', 'GIF');
    ...
    $perPage=24; // number of images to show per page
    ...
    $safe_url = htmlspecialchars($strSiteURL, ENT_QUOTES, 'UTF-8');
    $safe_directory = htmlspecialchars($directory, ENT_QUOTES, 'UTF-8');
    $safe_field = htmlspecialchars($field, ENT_QUOTES, 'UTF-8');
    print "Page ";
    for ($i=0;$i<$pages;$i++)
      if ($i==$thisPage)
        ...
        print "&nbsp;<a href='" . $safe_url . "/common/image.php?directory=" . urlencode($directory) . "&field=" . urlencode($field) . "&pg=" . ($i+1) . "'>".($i+1)."</a>";
    ...
    for ($i=$start;$i<$start+$perPage;$i++) {
      ...
      if (isset($list[$i])) {
        $safe_img_src = htmlspecialchars($url_image_directory . '/' . $list[$i], ENT_QUOTES, 'UTF-8');
        print "<td><a href=\"#\" class=\"imageSelector\"><img width='200px' height='auto' src='" . $safe_img_src . "'></a></td>";
      } else {
        print "<td></td>";
      }
...
?>
            ...
            $(function() {
                $('a.imageSelector').click(function() {
                    ...
                    parent.document.getElementById('<?php echo htmlspecialchars($field, ENT_QUOTES, 'UTF-8')?>').value = imagine;
