...
$stmt = mysqli_prepare($conn, "UPDATE tickets SET ticket_status=4, ticket_lastupdated=NOW(), ticket_lastupdatedby=? WHERE ticket_id=?");
mysqli_stmt_bind_param($stmt, "ii", $ui, $ticketId);
mysqli_stmt_execute($stmt);
mysqli_stmt_close($stmt);
log_action($ticketId, 'ADMIN', $ui, 'CLOSE_TICKET');
